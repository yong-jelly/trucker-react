        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-medium text-surface-900">단속 알고리즘 설정</h2>
            </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-soft-md border border-surface-100 space-y-6">
            <div className="space-y-6">
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-surface-700">기본 단속 발생 확률 (정속)</label>
                  <span className="text-sm font-medium text-primary-600">{settings.baseEnforcementRate}%</span>
                </div>
                <input 
                  type="range" 
                  min="0" max="100" 
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-surface-700">과속(Boost) 시 확률 가중치</label>
                  <span className="text-sm font-medium text-accent-rose">x{settings.speedingEnforcementMultiplier}</span>
                </div>
                <input 
                  type="range" 
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-surface-400 uppercase">야간 확률 보정</label>
                  <select className="w-full rounded-xl border border-surface-200 bg-surface-50 px-3 py-2 text-xs font-medium outline-none">
                    <option>낮음 (-5%)</option>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-surface-400 uppercase">지역 위험도</label>
                  <select className="w-full rounded-xl border border-surface-200 bg-surface-50 px-3 py-2 text-xs font-medium outline-none">
            ...
            <div className="pt-4 border-t border-surface-50 grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-medium text-surface-400 uppercase">기본 벌금 ($)</label>
                ...
                <p className="text-[10px] text-right text-surface-400 font-medium">${settings.baseFineAmount.toLocaleString()}</p>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-medium text-surface-400 uppercase">돌파 성공 확률 (%)</label>
                <div className="relative">
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-2xl bg-surface-900 p-5 font-mono text-[11px] space-y-3 shadow-2xl border border-white/5">
            <div className="space-y-1">
              ...
              <p className="text-accent-rose ml-4">└ <span className="font-medium">단속됨!</span> 벌금 <span className="underline">-$1,200</span> 및 평판 하락</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-white ml-4">└ <span className="text-surface-400">선택:</span> <span className="text-primary-400 font-medium underline">서류제시(Document)</span></p>
              <p className="text-accent-emerald ml-4">└ <span className="font-medium">성공!</span> 필수 서류 확인 완료 (무사 통과)</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-white ml-4">└ <span className="text-surface-400">선택:</span> <span className="text-primary-400 font-medium underline">우회(Bypass)</span></p>
              <p className="text-accent-blue ml-4">└ <span className="font-medium">회피 성공!</span> 경로 우회 완료 (ETA +12분)</p>
            </div>
          </div>
        </section>
      </div>
